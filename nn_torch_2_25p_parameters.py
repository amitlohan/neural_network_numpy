...
X_train,Y_train = get_cifar_2_classes(class1=6,class2=9)      #Reads data from CIFAR10 Batches for 5 Classes
X_test,Y_test = get_cifar_2_test_classes(class1=6,class2=9)   #Reads test data from CIFAR10 test Batch
...
class ConvNet(nn.Module):                            #Define the class for mode to be used for classication
    def __init__(self, num_classes=2):
        ...
        self.layer1 = nn.Sequential(                 #Define the first layer of model 
            nn.Conv2d(3,16, kernel_size=3, stride=1, padding=1), #Convolutional layer with input channel 3 and output channels 30
        ...
        self.layer3 = nn.Sequential(                 #Layer 3 with structure similar to above layers
            ...
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.fc = nn.Linear(4*4*32, num_classes)     #Fully connected layer with outputs equal to number of classes
